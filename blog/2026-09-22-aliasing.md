...
better way. I suspect this can be done by refining our alias sets to no longer
be sets of variable names, but variable names along with a *position*, which
allows us to alias only part of a tuple. If we then consume a tuple component,
then the tuple as a whole can no longer be referenced, but other unconsumed
subcomponents can still be used.
