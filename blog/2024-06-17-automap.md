...
something akin to NumPy-style broadcasting in Futhark. We've implemented an
[AUTOMAP prototype](https://github.com/diku-dk/futhark/tree/constraints) in
Futhark; AUTOMAP isn't yet available in `master` yet but we expect to merge it
in the nearish future. We've also written a paper on AUTOMAP (currently in
review), which I'm going to be giving a talk based on at
[ARRAY24](https://pldi24.sigplan.org/home/ARRAY-2024) on June 25th.

A super quick bio: I'm a PhD student here at [DIKU](https://di.ku.dk/) and I've
worked on Futhark one way or another for most of my PhD. (P.S. I'll be
graduating in December 2024 and I'm actively looking for potential postdoc
positions in all things type systems/functional programming/compilers---please
[get in touch](https://rschenck.com) if you think I might be a match!)
...
we had just published. In short, we were wrong and there was a surprising amount
of complexity involved, but we ended up with a really cool (and dare I say
elegant) solution. 

The motivation for the work was a desire to make mathematical code involving
higher-dimensional argument look closer to something you'd write by hand without
the clutter introduced by a bunch of "administrative" maps that are needed to
lift scalar operations to operate on the higher-dimensional arguments. For
example, if you want to add two vectors together represented by the arrays
`[1,2,3]` and `[4,5,6]`, you have to lift `+` to work over the arrays:

```Futhark
map2 (+) [1,2,3] [4,5,6]
```

If you were writing some mathematical notes by hand, you'd probably
just write:

    [1,2,3] + [4,5,6]

with the tacit understanding that what you really mean is the piecewise addition
of the two vectors.[^1] In fact, piecewise addition is really the only reasonable
thing to do here if the idea is to "make `+` work for these higher-dimensional
arguments"---shouldn't we be able to write `[1,2,3] + [4,5,6]` and have the
compiler figure out that we really meant `map2 (+) [1,2,3] [4,5,6]`? This isn't
only a boon for the lazy programmer; omitting the "noise" of the map also makes
things clearer and lets us use `+` in its proper infix location (sorry, Lisp
programmers).

## A few examples
As a first step, let's try to pin down what it is that we want the compiler to
do. To do that, let's look at another example---say, subtracting one matrix from
another:
...
for any number of arguments. So `map (+1) [1,2,3] = [2,3,4]` and `map (+) [1,2]
[3,4] = [4,6]`, etc. Hence the above can be expressed as just
...
have a rank mismatch with the arguments can be massaged into a rank-correct
...
The above application can be made rank-correct via inserting the following
`map`s and `rep`s
...
that were massaged/elaborated into
...
don't violate Rule 1 because functions are curried---the above is actually two
...
## Even with The Strategy, ambiguities can exist
Sometimes these rules aren't enough. Returning to our previous example involving
`sum` and `length`, observe that both
...
have the same number of inserted operations (one each) so both are minimal. When
there are multiple minimal options, we say that the program is *ambiguous*. In
this case, we have to ask the programmer for some help by signaling an
error. Because we're doing everything statically, we know the two possibilities
and can give the programmer a choice:
...
## All applications must be considered at simultaneously
...
elaboration of `sum (length xss)` is that we had to consider the outer application with
`sum` to know how to `map` the inner `length` application. In general, lifting
one application can affect others that depend on it because `map`ping changes the rank
of the application itself. The implication of this is quite significant:
inserting `map`s or `rep`s to make an application rank-correct requires
considering the entire top-level definition at hand (rather than just the local
application).
...
know the rank difference so we don't know how much to replicate it by yet
how many `rep`s we need), so we'll represent this unknown quantity with
the *rank variable* `R`:
...
again don't know by many `map`s we'll need, so we'll represent it with the rank
variable `M`:
...
Durng type checking, we accumulate these constraints into a big list. For
...
(Where we've added the subscript `_1` to the rank variables, since we'll have
...
rank to `M_1`, `R_1` and `|a|`.

Now we have to check the outer application involving `sum`: `sum (length xss)`.
...
the rank variable `M_1` above is assigned a non-zero value, then that means that
the application `length xss` has implicit maps---namely `M_1` of them.  This
means that (recalling that `length : []a -> int`) the type of the term `length
xss` will not be `int` but instead be an `M_1`-dimensional `int` array because
...
each implicit `map`: `length xss : []^M_1 int`, where `[]^M_1` is just a
sequence of `[]` of length `M_1` (and where we understand `length xss` to have
`M_1` implicit maps). If we end up inferring that no `map`s are to be inserted,
then `M_1 = 0` and we'll have `length xss : []^0 int` which is equivalent to
...
constraints for `sum (length xss : []^M_1 int)` because of the (potential)
...
Notice that the rank equality constraint depends on `M_1` (the `map` rank
...
Combining the constraints, our constraint set that expresses the rank
requirements for `sum (length xss)` are
...
What's left is to find a minimal assignment for `M_1`, `R_1`, `M_2`, `R_2`, and
...
using some linear programming trickery: for each pair of rank variables `M_i`
and `R_i`, we introduce two binary variables `b_M_i` and `b_R_i` that indicate
whether the application is to be `map`ped or `rep`ped. The idea is to enforce
that `M_i > 0` only if `b_M_i = 1` (and analogously for `R_i` and
`b_R_i`). Then, by requiring that `b_M_i + b_R_i <= 1`, we then enforce that
either `M_i = 0` or `R_i = 0`. To establish the requisite relationship between
`M_i` and `b_M_i`, we add the constraint

    M_i <= b_M_i * BIG_NUMBER
    
where `BIG_NUMBER` is just a big number. When `b_M_i = 0`, we have `M_i <= 0`
forcing it to be 0. When `b_M_i = 1`, we have `M_i <= BIG_NUMBER` which
effectively lets the solver assign `M_i` to be any rank-satisfying value (as
long as `BIG_NUMBER` is bigger than any rank we expect to see in a program---in
practice ranks are small so isn't a big deal). We do something analogous for
`R_i` and `b_R_i` to obtain the linearized minimization problem
    ...
    subject to
      M_1 + 1 + |a| = R_1 + 2
      b_M_1 + b_R_1 <= 1
      M_1 <= b_M_1 * BIG_NUMBER
      R_1 <= b_R_1 * BIG_NUMBER
      M_2 + 1 = R_2 + M_1
      b_M_2 + b_R_2 <= 1
      M_2 <= b_M_2 * BIG_NUMBER
      R_2 <= b_R_2 * BIG_NUMBER
...
3. The solution to the ILP tells the compiler how to insert the minimal number
   of `map`s and `rep`s to make the program rank-correct. For example, if `M_i =
   3` and `R_i = 0` for an application `f x`, then transform the application by
...
to use the rank polymorphism system to whatever extent that they wish---from
omitting all `map`s and `rep`s (up to ambiguity), to only some, to having a
program with all `map`s and `rep`s explicit.
...
[^2]: In most languages with rank polymorphism, this isn't a property of function
applications but rather functions *themselves*.  Typed array polymorphic
...
makes type inference very, very challenging.
