...
each implicit `map`: `length xss : []^M₁ int`, where `[]^M₁` is just a
...
then `M₁ = 0` and we'll have `length xss : []^0 int` which is equivalent to
...
constraints for `sum (length xss : []^M₁ int)` because of the (potential)
